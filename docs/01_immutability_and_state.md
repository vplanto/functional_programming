...
4. **У [Лекції 00](00_intro_and_ai.md) ми запропонували аналогію «скриньки» замість черги з Lock. Чому цього недостатньо, якщо мільйон людей хочуть одне VIP-місце?**
   <details markdown="1">
   <summary>Відповідь</summary>
   
   Скринька прибирає блокування на запис, але не вирішує конфлікт за дефіцитний ресурс. Потрібен механізм арбітражу: кожна заявка стає незмінною подією з міткою часу, обробники сортують їх за timestamp і детерміновано застосовують по черзі. Перша за часом отримує квиток, решта — відмову. Immutability і Event Sourcing — не срібна куля, а архітектурний компроміс із глобальною синхронізацією часу.
   </details>
...
## 3. Незмінність — не срібна куля: вирішення конфліктів через глобальний час

У [попередній лекції](00_intro_and_ai.md) ми говорили про кейс Ticketmaster і запропонували альтернативу блокувань: замість черги до кабінету — **скринька**, куди кожен кидає асинхронну заявку «хочу купити квиток», і система обробляє ці події без очікування на Lock.

Але після лекції у багатьох студентів виникає логічне заперечення: *«Добре, скринька приймає мільйон заявок миттєво. Але якщо всі вони — на одне-єдине VIP-місце, як ми не продамо цей квиток мільйон разів, якщо ми не блокуємо базу даних?»*

Це справедливе питання. **Незмінність і Event Sourcing — не срібна куля.** Вони прибирають черги на запис, але не скасовують фізичну реальність: квиток один, а бажаючих — мільйон. Секрет вирішення цієї проблеми в розподілених системах лежить у **глобальній синхронізації часу**.

### 1. Проблема точного часу

У розподіленій системі сервери розкидані по різних континентах. Отримати ідеально однаковий час на всіх вузлах звичайними методами неможливо — мережеві затримки, дрейф годинників і різниця в часових поясах завжди залишаються.

Тому такі гіганти, як Google, використовують атомні годинники та GPS-синхронізацію в дата-центрах. Замість «точної мілісекунди» кожен сервер отримує **гарантований мікро-інтервал**, у якому зараз знаходиться час. Цього достатньо, щоб упорядкувати мільйони подій без спільного Lock.

### 2. Мітка часу як єдиний арбітр

Коли мільйон користувачів тиснуть «Купити», їхні запити **не намагаються одразу змінити** кількість квитків у базі. Кожен запит перетворюється на **Подію (Event)** — незмінний факт — і отримує надточну мітку часу (timestamp) від синхронізованого сервера.

```scala
// Не мутація: "availableSeats = 0"
// А незмінний факт:
case class TicketPurchaseRequested(userId: String, seatId: String, timestamp: Long)
```

Саме тут незмінність з попередніх розділів зустрічається з реальною архітектурою: ми не перезаписуємо рядок `available_vip_seats`, ми **додаємо новий запис у журнал подій**.

### 3. Детермінована обробка

Усі ці події злітаються у спільний пул — **Event Stream**. Далі спеціальний пул серверів-обробників сортує заявки **строго за їхнім часом виникнення**:

1. **Перша заявка** (найраніший timestamp): обробник перевіряє, чи є квиток. Є → генерується подія `TicketSold`, квиток списується.
2. **Друга заявка** (на мілісекунду пізніше): квитків уже 0 → генерується подія `TicketRejected`.
3. **Якщо перша транзакція фейлиться** (наприклад, не пройшла оплата): квиток повертається в пул, і його забирає наступна за часом заявка.

```mermaid
sequenceDiagram
    participant U1 as Користувач 1
    participant U2 as Користувач 2
    participant ES as Event Stream
    participant P as Обробник

    U1->>ES: PurchaseRequested (t=1000)
    U2->>ES: PurchaseRequested (t=1001)
    P->>ES: Сортування за timestamp
    P->>P: t=1000 → TicketSold ✓
    P->>P: t=1001 → TicketRejected ✗
```

### Підсумок: що ми отримуємо і чого не отримуємо

**Що вирішує цей підхід:**
- Ми уникаємо Race Condition і блокувань (Locks) бази даних. Ніхто нікого не чекає в черзі на запис.
- Усі заявки фіксуються миттєво — система не «падає» від каскаду ретраїв, як у Ticketmaster.
- Справедливість (хто отримав квиток) визначається фізичним часом натискання кнопки, підтвердженим глобальними годинниками. Ті, хто запізнився на мілісекунду, побачать «Вибачте, квитки розкуплені».

**Чого цей підхід не дає безкоштовно:**
- Потрібна інфраструктура синхронізації часу (це не «просто скринька»).
- Користувач бачить результат не миттєво, а після обробки потоку подій — тимчасова **Eventual Consistency**, про яку ми говорили в [Лекції 00](00_intro_and_ai.md).
- Справедливість «хто перший натиснув» залежить від якості синхронізації годинників — це інженерний компроміс, а не магія ФП.

> **Головний висновок лекції:** Immutability прибирає конфлікти *на читання* і дозволяє безпечно паралелити обчислення. Але коли мільйони агентів змагаються за *один* ресурс, потрібен окремий механізм арбітражу — і в індустрії цим арбітром часто стає **упорядкований журнал подій із глобальними мітками часу**.

## 4. Архітектурний патерн "Functional Core, Imperative Shell"
...
3. **Чому Event Sourcing із «скринькою» — не срібна куля, і як система вирішує конфлікт за одне VIP-місце без Locks?**
   <details markdown="1">
   <summary>Відповідь</summary>
   
   Immutability прибирає конфлікти на читання, але не скасовує фізичний дефіцит ресурсу. Кожен запит стає незмінною подією з міткою часу від синхронізованих серверів. Обробники сортують події за timestamp і детерміновано застосовують їх по черзі: перша за часом — `TicketSold`, решта — `TicketRejected`. Справедливість визначає глобальний час, а не Lock у базі даних.
   </details>
